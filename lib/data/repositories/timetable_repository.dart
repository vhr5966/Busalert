...
class TimetableRepository {
  ...
  /// Fetches real-time timetable entries for a given [stop] and optional [routeNumber] and [directionId].
  ...
  List<TimetableEntry> _generateFallbackTimetable(
    ...
    // Stop-specific hash offset so different stops along the route have distinct, realistic departure times
    final stopHashOffset = (stop.name.hashCode.abs() % 11) + 2;
    int offsetMinutes = stopHashOffset;
    ...
    for (final route in selectedRoutes) {
      ...
      final headsign = RouteDestinationResolver.resolveDestination(
        routeNumber: route,
        stopName: stop.name,
        rawHeadsign: isLive ? liveMatch.formattedDestination : null,
      );
