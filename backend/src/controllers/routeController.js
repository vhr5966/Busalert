...
exports.getRoutes = async (req, res) => {
  try {
    ...
    // ── Priority 3: Fallback to official 64 GTFS routes ────────────────
    if (parsedRoutes.length === 0) {
      const ROUTES_FILE = require('path').join(__dirname, '../../data/routes.json');
      if (require('fs').existsSync(ROUTES_FILE)) {
        try {
          parsedRoutes = JSON.parse(require('fs').readFileSync(ROUTES_FILE, 'utf8'));
          source = 'Official GTFS Reference Routes (Cardiff Bus & Newport Bus)';
        } catch (err) {
          console.error('Error loading fallback routes.json:', err.message);
        }
      }
    }
